...
2. **Take the legal pages to a lawyer.** The privacy page lists six open items on the page itself. Two more are recorded in the pull request: the age of digital consent is set to 16 while Spain's is 14, and the processor list needs an agreement with each named party.
3. **Approve the coaching drills.** Ten drills ship as reviewed but not approved, and the session assembler correctly refuses to build a session from unapproved material. Approval is a deliberate human decision inside the app.
4. **Buy a domain and point it at the server.** The app currently answers on an IP-derived hostname.
